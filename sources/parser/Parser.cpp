...
/*   Updated: 2023/01/25 12:13:52 by mvieira-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// The first server for a host:port will be the default for this host:port (that
// means it will answer to all the requests that don’t belong to an other
// server). The routes configuration.
...
int Parser::parse()
{
    ...
    if ( !file.is_open() ) {
        std::cout << "Error opening configuration file: " << this->file_name
                  << std::endl;
        return 1;
    }
